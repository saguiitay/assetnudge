export default {
  "slug": "tools-animation",
  "name": "Tools/Animation",
  "description": "Guidelines for creating standout Tools/Animation assets on the Unity Asset Store.",
  "overview": {
    "marketSize": "Small–Medium – a few hundred assets",
    "competition": "Moderate competition – strong previews and keywords help",
    "averagePrice": "Around $27.0"
  },
  "recommendations": {
    "title": {
      "optimalLength": "20–29 characters (3–4 words)",
      "tips": [
        "Lead with the tool type and target (e.g., 'Foot IK System for Humanoids').",
        "Mention workflow or performance benefits if space allows.",
        "Avoid repeating buzzwords; be specific about capability.",
        "Front-load primary keywords and the asset type.",
        "Avoid filler words like 'Ultimate' unless you can prove scope.",
        "Use Title Case for readability."
      ],
      "examples": {
        "good": [
          "Quick Animation Events - Manage Animation Events Easily",
          "HK Foot Placement V3 - Foot IK system - Triple - A quality",
          "HK Foot Placement V3 - Foot IK system - Triple - A quality",
          "Motion Matching for Unity",
          "Anything World 3D Creation & Animation Studio"
        ],
        "bad": [
          "Head Look Controller",
          "Head Look Controller",
          "Skinning to BlendShapes"
        ]
      }
    },
    "description": {
      "optimalLength": "120–250 words",
      "structure": [
        "What it does and who it’s for (1–2 sentences).",
        "Key features and supported rigs/skeletons.",
        "Performance and pipeline details (Animator, Timeline, DOTS if relevant).",
        "Setup steps and demo scenes.",
        "Support and update policy."
      ],
      "tips": [
        "Explain exactly what’s included with counts, formats, and variations.",
        "State supported pipelines/Unity versions and any requirements.",
        "Add 2–4 concrete use cases so buyers can picture fit.",
        "Link to a demo scene or quickstart steps.",
        "Keep paragraphs short and scannable."
      ],
      "example": "Bring believable results to your project with a curated, ready-to-use set. You’ll get multiple variations, clear folder structure, and support for common Unity pipelines (URP/HDRP or Built-in). Designed for quick iteration, this pack blends quality with performance and ships with a small demo scene and usage tips so you can drag-and-drop and ship faster."
    },
    "images": {
      "optimalCount": "4–7 images",
      "requirements": [
        "Short GIF/video strongly recommended for motion.",
        "Editor screenshots for workflows and inspectors.",
        "Before/after or comparison shots for clarity."
      ],
      "tips": [
        "Lead with a clean hero that summarizes the value.",
        "Use consistent framing and typography across images.",
        "Include at least one real in-engine screenshot.",
        "Avoid busy collages—keep each image focused on one idea."
      ]
    },
    "tags": {
      "optimalCount": "8–12 relevant tags",
      "commonTags": [
        "Animation",
        "Tool",
        "Animated",
        "Animator",
        "IK",
        "Procedural Animation",
        "Character",
        "Editor",
        "tween",
        "Timeline"
      ],
      "tips": [
        "Mix generic discovery tags (e.g., 'Environment') with specific ones that describe content or workflow.",
        "Match pipelines and formats (URP/HDRP, PBR) where applicable.",
        "Avoid repeating the same word with different casing."
      ]
    },
    "keywords": {
      "primary": [
        "foot placement",
        "animation events",
        "make your",
        "placement foot",
        "foot system",
        "system triple",
        "triple quality",
        "quality advanced",
        "advanced foot",
        "placement make"
      ],
      "secondary": [
        "head look",
        "look controller",
        "head look controller",
        "animation system",
        "animation tool",
        "make your",
        "animation events",
        "easy use",
        "allows you",
        "your game"
      ],
      "tips": [
        "Use primary keywords in the first 40 characters of your title.",
        "Repeat key phrases naturally in the first 2–3 lines of the description.",
        "Align tags with the same language to reinforce search relevance."
      ]
    },
    "pricing": {
      "range": "$4.99–$500",
      "strategy": [
        "Anchor price to scope and clarity of value (counts, variants, support).",
        "Offer a free lite/demo to drive traffic without underpricing the main pack.",
        "Consider bundles or upgrade paths as your catalog grows.",
        "Participate in seasonal sales with clear before/after pricing."
      ]
    }
  },
  "realExamples": [
    {
      "name": "Quick Animation Events - Manage Animation Events Easily",
      "publisher": "Harpia Games Studio",
      "tags": [
        "Game Events",
        "humanoid animations",
        "Unity Event",
        "Events",
        "Animation",
        "animation event",
        "Animation tools",
        "Event System",
        "avatar",
        "Easy",
        "Fast",
        "Workflow"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Strong visuals with multiple in-engine screenshots.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 55,
        "descriptionLength": 150,
        "imageCount": 8,
        "tagCount": 12
      }
    },
    {
      "name": "HK Foot Placement V3 - Foot IK system - Triple - A quality",
      "publisher": "HK(Hamza Khan)",
      "tags": [
        "Placement",
        "grounder",
        "IK",
        "foot",
        "Humanoid foot placement",
        "body placement",
        "placer",
        "advanced",
        "foot IK",
        "HK",
        "inverse Kinematics",
        "Foot placement ik"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Adequate image gallery that shows real usage.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 58,
        "descriptionLength": 182,
        "imageCount": 5,
        "tagCount": 15
      }
    },
    {
      "name": "Motion Matching for Unity",
      "publisher": "Vault Break Studios",
      "tags": [
        "Animation",
        "Motion",
        "Motion Matching",
        "Animator",
        "Character",
        "Mecanim"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Strong visuals with multiple in-engine screenshots.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 25,
        "descriptionLength": 178,
        "imageCount": 10,
        "tagCount": 6
      }
    },
    {
      "name": "Anything World 3D Creation & Animation Studio",
      "publisher": "Anything World",
      "tags": [
        "3D character animation",
        "3D animation",
        "Animation",
        "animated humanoids",
        "Animated Animals",
        "machine learning animation",
        "ai animation",
        "Character Animation",
        "ai voice",
        "AI",
        "animated humans",
        "animations"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Adequate image gallery that shows real usage.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 45,
        "descriptionLength": 199,
        "imageCount": 7,
        "tagCount": 15
      }
    },
    {
      "name": "FPS Animation Framework",
      "publisher": "KINEMATION",
      "tags": [
        "Procedural Animation",
        "FPS",
        "FPS Kit",
        "Fps Controller",
        "fps animation",
        "Animation",
        "Tool",
        "Procedural",
        "inverse Kinematics",
        "first person shooter",
        "Shooter",
        "First Person"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Strong visuals with multiple in-engine screenshots.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 23,
        "descriptionLength": 53,
        "imageCount": 9,
        "tagCount": 14
      }
    }
  ],
  "commonMistakes": [
    {
      "mistake": "Thin or missing description",
      "impact": "Buyers can’t tell what’s included or if it fits their project, so they bounce.",
      "solution": "Write 120–250 words covering what it is, what’s included (counts, formats), compatibility, and practical examples."
    },
    {
      "mistake": "Too few or generic preview images",
      "impact": "Low confidence and poor discoverability on category pages.",
      "solution": "Show 6–10 images: hero shots, close-ups, real in-engine views, and a simple infographic of what’s included."
    },
    {
      "mistake": "Missing or irrelevant tags",
      "impact": "Search and category filters won’t surface your asset to the right audience.",
      "solution": "Add 8–12 relevant tags matching genre, pipeline, resolution, and use cases."
    },
    {
      "mistake": "Outdated listing with no recent updates",
      "impact": "Signals poor maintenance and may break on newer Unity versions/pipelines.",
      "solution": "Ship a small update (URP/HDRP notes, demo scene refresh) and highlight the new support in the changelog."
    }
  ]
}