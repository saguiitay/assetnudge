export default {
  "slug": "tools-ai-ml-integration",
  "name": "Tools/AI-ML Integration",
  "description": "Guidelines for creating standout Tools/AI-ML Integration assets on the Unity Asset Store.",
  "overview": {
    "marketSize": "Very small – fewer than 100 assets",
    "competition": "Lower competition – niche quality can stand out",
    "averagePrice": "Around $52.0"
  },
  "recommendations": {
    "title": {
      "optimalLength": "23–45 characters (3–6 words)",
      "tips": [
        "Lead with the platform and outcome (e.g., 'GPT Dialogue Tools for Unity').",
        "Call out key integrations (OpenAI, Whisper, ElevenLabs) if space allows.",
        "Avoid buzzwords; focus on what it enables.",
        "Front-load primary keywords and the asset type.",
        "Avoid filler words like 'Ultimate' unless you can prove scope.",
        "Use Title Case for readability."
      ],
      "examples": {
        "good": [
          "Dialogue System for Unity Addon for OpenAI, ElevenLabs & Other Generative AI",
          "VR Gesture Detection",
          "Spark AI - GPT Dialogue & Tools",
          "GPT AI Integration",
          "Automation Toolkit: Custom Bridge"
        ],
        "bad": [
          "Emgu TF Lite v2.x",
          "Native Speech Recognition",
          "Native Speech Recognition"
        ]
      }
    },
    "description": {
      "optimalLength": "120–250 words",
      "structure": [
        "Who it’s for and the core value (1–2 sentences).",
        "Key features and supported services/APIs.",
        "Setup steps and editor workflow.",
        "Compatibility (Unity versions, platforms).",
        "Support policy and examples."
      ],
      "tips": [
        "Explain exactly what’s included with counts, formats, and variations.",
        "State supported pipelines/Unity versions and any requirements.",
        "Add 2–4 concrete use cases so buyers can picture fit.",
        "Link to a demo scene or quickstart steps.",
        "Keep paragraphs short and scannable."
      ],
      "example": "Bring believable results to your project with a curated, ready-to-use set. You’ll get multiple variations, clear folder structure, and support for common Unity pipelines (URP/HDRP or Built-in). Designed for quick iteration, this pack blends quality with performance and ships with a small demo scene and usage tips so you can drag-and-drop and ship faster."
    },
    "images": {
      "optimalCount": "5–9 images",
      "requirements": [
        "Editor UI screenshots and workflow diagrams.",
        "Before/after clips or short tutorial video.",
        "Clear feature list slides."
      ],
      "tips": [
        "Lead with a clean hero that summarizes the value.",
        "Use consistent framing and typography across images.",
        "Include at least one real in-engine screenshot.",
        "Avoid busy collages—keep each image focused on one idea."
      ]
    },
    "tags": {
      "optimalCount": "8–12 relevant tags",
      "commonTags": [
        "AI",
        "machine learning",
        "ChatGPT",
        "OpenAI",
        "artificial intelligence",
        "Editor Tools",
        "whisper",
        "speech recognition",
        "Computer Vision",
        "Chat"
      ],
      "tips": [
        "Mix generic discovery tags with specific ones that describe content or workflow.",
        "Match pipelines and formats (URP/HDRP, PBR) where applicable.",
        "Avoid repeating the same word with different casing."
      ]
    },
    "keywords": {
      "primary": [
        "system unity",
        "real time",
        "dialogue system",
        "gesture detection",
        "unity real",
        "time gesture",
        "gesture recognition",
        "spark gpt",
        "gpt dialogue",
        "your unity"
      ],
      "secondary": [
        "real time",
        "hand finger",
        "finger tracking",
        "hand finger tracking",
        "unity editor",
        "speech recognition",
        "google cloud",
        "using google",
        "using google cloud",
        "computer vision"
      ],
      "tips": [
        "Use primary keywords in the first 40 characters of your title.",
        "Repeat key phrases naturally in the first 2–3 lines of the description.",
        "Align tags with the same language to reinforce search relevance."
      ]
    },
    "pricing": {
      "range": "$4.99–$490",
      "strategy": [
        "Anchor price to scope and clarity of value (counts, variants, support).",
        "Offer a free lite/demo to drive traffic without underpricing the main pack.",
        "Consider bundles or upgrade paths as your catalog grows.",
        "Participate in seasonal sales with clear before/after pricing."
      ]
    }
  },
  "realExamples": [
    {
      "name": "Dialogue System for Unity Addon for OpenAI, ElevenLabs & Other Generative AI",
      "publisher": "Pixel Crushers",
      "tags": [
        "translation",
        "Dialogue",
        "OpenAI",
        "Generation",
        "AI",
        "Open AI",
        "Chat GPT",
        "ChatGPT",
        "gpt",
        "whisper",
        "Generative",
        "DALL-e"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Strong visuals with multiple in-engine screenshots.",
        "Calls out pipeline or integration details to reduce uncertainty.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 76,
        "descriptionLength": 198,
        "imageCount": 14,
        "tagCount": 15
      }
    },
    {
      "name": "VR Gesture Detection",
      "publisher": "InventaPlay Studio",
      "tags": [
        "VR Gesture Detection",
        "VR Sports Simulation",
        "Real-time Motion Tracking",
        "hand gesture recognition",
        "Natural VR Interactions",
        "AR Gesture Detection",
        "VR motion detection",
        "Unity XR Framework",
        "Immersive VR Controls",
        "Machine Learning for VR",
        "gesture recognition",
        "VR Interaction System"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Strong visuals with multiple in-engine screenshots.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 20,
        "descriptionLength": 198,
        "imageCount": 16,
        "tagCount": 14
      }
    },
    {
      "name": "Spark AI - GPT Dialogue & Tools",
      "publisher": "LeastSquares LLC",
      "tags": [
        "ChatGPT",
        "AI",
        "OpenAI",
        "Dialogue",
        "whisper",
        "transcribe",
        "GPT4",
        "transcription",
        "Editor Tools",
        "image generation",
        "DALLE",
        "Chat"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Strong visuals with multiple in-engine screenshots.",
        "Calls out pipeline or integration details to reduce uncertainty.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 31,
        "descriptionLength": 192,
        "imageCount": 10,
        "tagCount": 15
      }
    },
    {
      "name": "GPT AI Integration",
      "publisher": "Cameron Wills",
      "tags": [
        "artificial intelligence",
        "OpenAI",
        "AI",
        "Chat",
        "machine learning",
        "language",
        "multi-language",
        "Editor Tools",
        "Visual Scripting",
        "ChatGPT",
        "GPT4",
        "GPT3.5"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Strong visuals with multiple in-engine screenshots.",
        "Calls out pipeline or integration details to reduce uncertainty.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 18,
        "descriptionLength": 200,
        "imageCount": 10,
        "tagCount": 15
      }
    },
    {
      "name": "Automation Toolkit: Custom Bridge",
      "publisher": "POWXR",
      "tags": [
        "AI-Integration",
        "toolkit",
        "zapier",
        "webhook",
        "automation toolkit",
        "automation",
        "n8n",
        "make",
        "n8n unity",
        "OpenAI Unity",
        "Automation Unity Toolkit",
        "Unity n8n"
      ],
      "whyItWorks": [
        "Clear, specific title that sets expectations.",
        "Concise description that highlights what's included and where it fits.",
        "Adequate image gallery that shows real usage.",
        "Calls out pipeline or integration details to reduce uncertainty.",
        "Includes a video preview for quick evaluation."
      ],
      "metrics": {
        "titleLength": 33,
        "descriptionLength": 200,
        "imageCount": 6,
        "tagCount": 14
      }
    }
  ],
  "commonMistakes": [
    {
      "mistake": "Thin or missing description",
      "impact": "Buyers can’t tell what’s included or if it fits their project, so they bounce.",
      "solution": "Write 120–250 words covering what it is, what’s included (counts, formats), compatibility, and practical examples."
    },
    {
      "mistake": "Too few or generic preview images",
      "impact": "Low confidence and poor discoverability on category pages.",
      "solution": "Show 6–10 images: hero shots, close-ups, real in-engine views, and a simple infographic of what’s included."
    },
    {
      "mistake": "Overpriced vs. perceived scope",
      "impact": "Shoppers compare quickly and skip assets that feel expensive for what they get.",
      "solution": "Align price with scope; add bundles or tiers and clarify value (counts, support, performance)."
    },
    {
      "mistake": "Underpricing that undermines quality",
      "impact": "Very low prices can imply low quality and reduce trust.",
      "solution": "Price fairly for the content and include a free lite sample instead of discounting the main pack."
    }
  ]
}